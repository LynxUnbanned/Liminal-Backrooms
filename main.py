...
def ai_turn(ai_name, conversation, model, system_prompt, gui=None, is_branch=False, branch_output=None, streaming_callback=None):
    ...
    try:
        ...
        # Direct Gemini models (non-OpenRouter)
        if model_id.startswith("gemini") and "/" not in model_id:
            print(f"Using Gemini API for model: {model_id}")
        ...
        # Direct Groq models
        groq_ids = {"moonshotai/kimi-k2-instruct-0905", "openai/gpt-oss-120b", "openai/gpt-oss-20b"}
        if model.lower().startswith("groq") or model_id.startswith("groq/") or model_id in groq_ids:
            print(f"Using Groq API for model: {model_id}")
            ...
            response = call_groq_api(prompt_content, context_messages, model_id, system_prompt, stream_callback=streaming_callback)
        ...
        cerebras_ids = {"gpt-oss-120b", "qwen-3-235b-a22b-instruct-2507", "zai-glm-4.6"}
        if model.lower().startswith("cerebras") or model_id.startswith("cerebras/") or model_id in cerebras_ids:
            print(f"Using Cerebras API for model: {model_id}")
            ...
            response = call_cerebras_api(prompt_content, context_messages, model_id, system_prompt, stream_callback=streaming_callback)
...
class ConversationManager:
    ...
    def process_branch_input(self, user_input=None):
        ...
        cp = self.app.right_sidebar.control_panel
        ai_1_model = cp.get_model_key(cp.ai1_model_selector)
        ai_2_model = cp.get_model_key(cp.ai2_model_selector)
        ai_3_model = cp.get_model_key(cp.ai3_model_selector)
        selected_prompt_pair = self.app.right_sidebar.control_panel.prompt_pair_selector.currentText()
    ...
    def _execute_add_ai_command(self, model_name: str, persona: str, requesting_ai: str) -> tuple[bool, str]:
        ...
        if not requested:
            return False, "No model specified for !add_ai"

        def resolve_model(requested_model: str):
            norm = requested_model.lower()
            for display, model_id in AI_MODELS.items():
                if norm == display.lower() or norm == model_id.lower() or norm in display.lower():
                    return display, model_id
            return None, None

        display_name, model_id = resolve_model(requested)
        if not model_id:
            available = "\n".join(AI_MODELS.keys())
            return False, f"Model not recognized. Available models:\n{available}"
        ...
        actual_model = model_id  # Track the model id that will be used
        actual_display = display_name
        selector = getattr(self.app.right_sidebar.control_panel, f'ai{new_num}_model_selector', None)
        if selector:
            # Find if the requested model exists in the selector by userData (model id)
            found = False
            for i in range(selector.count()):
                if selector.itemData(i) and str(selector.itemData(i)).lower() == model_id.lower():
                    selector.setCurrentIndex(i)
                    actual_display = selector.itemText(i)
                    actual_model = selector.itemData(i)
                    found = True
                    break
            if not found:
                # Fallback: leave selector as-is but use resolved model_id
        ...
        for i in range(1, base_num_ais + 1):
            ...
            if existing_selector:
                existing_model = self.app.right_sidebar.control_panel.get_model_key(existing_selector)
                if existing_model and existing_model.lower() == actual_model.lower():
                    print(f"[Agent] {actual_model} already active as AI-{i}, skipping duplicate")
                    return True, f"✨ {display_name} is already in the conversation as AI-{i}"
        
        # Check if this model was already invited this round (pending deduplication)
        already_pending = any(p['model'].lower() == actual_model.lower() for p in self._pending_ais)
        if already_pending:
            print(f"[Agent] {actual_model} already invited this round, skipping duplicate")
            return True, f"✨ {display_name} was already invited (by another AI)"
        ...
        if persona:
            return True, f"✨ [{requesting_ai}]: !add_ai \"{display_name}\" \"{persona}\""
        else:
            return True, f"✨ [{requesting_ai}]: !add_ai \"{display_name}\""
    ...
    def get_model_for_ai(self, ai_number):
        ...
        # Use stored model key, falling back to stripping provider suffix
        return self.app.right_sidebar.control_panel.get_model_key(selector)
