...
class ControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        """)
        self.auto_image_checkbox.setToolTip("Automatically generate images from AI responses using Cloudflare Workers AI")
    ...
    def initialize_selectors(self):
        ...
        def provider_for_model(model_name, model_id):
            """Return a short provider label based on the model id/name."""
            mid = (model_id or "").lower()
            if mid.startswith("gemini") or "google/gemini" in mid:
                return "Gemini"
            if mid.startswith("moonshotai/") or mid.startswith("openai/gpt-oss"):
                return "Groq"
            if mid.startswith("cerebras/") or mid in {"gpt-oss-120b", "qwen-3-235b-a22b-instruct-2507", "zai-glm-4.6"}:
                return "Cerebras"
            # OpenRouter set
            if any(p in mid for p in ["x-ai/", "tngtech/", "z-ai/", "qwen/", "cognitivecomputations/"]):
                return "OpenRouter"
            return "Other"

        def add_models(selector):
            for model_name, model_id in AI_MODELS.items():
                provider = provider_for_model(model_name, model_id)
                display_text = f"{model_name} · {provider}"
                selector.addItem(display_text, userData=model_id)
        ...
        add_models(self.ai1_model_selector)
        add_models(self.ai2_model_selector)
        add_models(self.ai3_model_selector)
        add_models(self.ai4_model_selector)
        add_models(self.ai5_model_selector)
    ...
    def get_model_key(self, selector: QComboBox) -> str:
        """Return the underlying model key stored on the selector item."""
        data = selector.currentData()
        if data:
            return data  # model_id stored as userData
        # Fallback: strip provider suffix if user data missing
        text = selector.currentText() or ""
        display_name = text.split(" · ")[0].strip()
        # Map back via config if possible
        return AI_MODELS.get(display_name, display_name)
